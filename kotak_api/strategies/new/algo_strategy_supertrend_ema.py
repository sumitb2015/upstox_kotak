...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(LOG_DIR, "algo_strategy_supertrend_ema.log"), encoding='utf-8'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("SupertrendEMA")
...
STATE_FILE = os.path.join(DATA_DIR, "supertrend_ema_state.json")
...
class SupertrendEMAStrategy:
    ...
    def calculate_indicators(self, df):
        try:
            if len(df) < max(ST_PERIOD, EMA_PERIOD) + 5: return None
            ...
            return {
                'st': st_df.iloc[-2][f'SUPERT_{ST_PERIOD}_{ST_MULTIPLIER}'],
                'ema': ema_series.iloc[-2],
                'candle_close': df['Close'].iloc[-2],
                'candle_open': df['Open'].iloc[-2],
    ...
    def check_entry(self, spot_ltp, inds):
        """
        Trend Following: Nifty vs Supertrend & EMA 10
        + Candle Color Confirmation
        """
        st_val = inds['st']
        ema_val = inds['ema']
        close = inds['candle_close']
        open_price = inds['candle_open']
        
        # SELL CE (Bearish)
        # Price below Supertrend AND Price below EMA 10 AND Red Candle
        if close < st_val and close < ema_val and close < open_price:
            logger.info(f"🚀 BEARISH: Close ({close:.1f}) < ST ({st_val:.1f}) & EMA10 ({ema_val:.1f}) & Red Candle. SELL CE")
            self.execute_entry("CE_SHORT", spot_ltp, st_val) # Using ST as visual stop ref

        # SELL PE (Bullish)
        # Price above Supertrend AND Price above EMA 10 AND Green Candle
        elif close > st_val and close > ema_val and close > open_price:
            logger.info(f"🚀 BULLISH: Close ({close:.1f}) > ST ({st_val:.1f}) & EMA10 ({ema_val:.1f}) & Green Candle. SELL PE")
            self.execute_entry("PE_SHORT", spot_ltp, st_val)
    ...
    def run(self):
        logger.info("Supertrend EMA Strategy Started...")
