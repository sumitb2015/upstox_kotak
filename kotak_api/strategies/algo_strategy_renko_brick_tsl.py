...
# Configure Logging
logger = setup_strategy_logger("RenkoBrickTSLStrategy", "algo_strategy_renko_brick_tsl.log")
...
class RenkoStrategy:
        ...
        while self.running:
            try:
                ...
                if time.time() - last_heartbeat > 10:
                    # Calculate reversal SL level based on BEST levels (trailing stop)
                    # Note: Best levels are updated ONLY when bricks form (true Renko logic)
                    reversal_sl_info = ""
                    if self.entry_state == "IN_GREEN_TREND" and self.best_nifty_high > 0:
                        # In Green Trend (Short PE): Reversal = 2 Red bricks from BEST HIGH
                        # Stop locks profit as market rises, never moves down
                        reversal_level = self.best_nifty_high - (2 * BRICK_SIZE)
                        reversal_sl_info = f" | Reversal SL: {reversal_level:.2f} (locked @ {self.best_nifty_high:.2f})"
                    elif self.entry_state == "IN_RED_TREND" and self.best_nifty_low < 999999:
                        # In Red Trend (Short CE): Reversal = 2 Green bricks from BEST LOW
                        # Stop locks profit as market falls, never moves up
                        reversal_level = self.best_nifty_low + (2 * BRICK_SIZE)
                        reversal_sl_info = f" | Reversal SL: {reversal_level:.2f} (locked @ {self.best_nifty_low:.2f})"
    ...
    def on_new_brick(self):
        """Handle Signal Logic and Update Trailing Stop (Brick-Based)"""
        ...
        # Update trailing stop levels ONLY on brick formation (true Renko logic)
        if self.entry_state == "IN_GREEN_TREND" and last_brick.color == 'GREEN':
            # For Short PE: Track highest close (brick formation)
            if last_brick.close > self.best_nifty_high:
                old_best = self.best_nifty_high
                self.best_nifty_high = last_brick.close
                logger.info(f"  📈 TSL Updated: Best High {old_best:.2f} → {self.best_nifty_high:.2f} | SL: {self.best_nifty_high - (2 * BRICK_SIZE):.2f}")
        elif self.entry_state == "IN_RED_TREND" and last_brick.color == 'RED':
            # For Short CE: Track lowest close (brick formation)
            if last_brick.close < self.best_nifty_low:
                old_best = self.best_nifty_low
                self.best_nifty_low = last_brick.close
                logger.info(f"  📉 TSL Updated: Best Low {old_best:.2f} → {self.best_nifty_low:.2f} | SL: {self.best_nifty_low + (2 * BRICK_SIZE):.2f}")
