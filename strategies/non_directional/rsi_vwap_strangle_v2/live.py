...
from strategies.non_directional.rsi_vwap_strangle_v2.config import *
...
class RSIVWAPStrangle:
    ...
    def initialize(self):
        ...
        self.expiry_dt = get_expiry_for_strategy(self.upstox_token, EXPIRY_TYPE, short_symbol)
    ...
    def enter_strategy(self):
        ...
        ce_key = get_option_instrument_key(self.short_symbol, ce_strike, "CE", self.nse_data, expiry_date=self.expiry_dt)
        pe_key = get_option_instrument_key(self.short_symbol, pe_strike, "PE", self.nse_data, expiry_date=self.expiry_dt)
    ...
    def manage_position(self):
        ...
        # 1. Stop Loss Check (Two-Tier: Combined Gate -> Individual Check)
        tsl_pct = PYRAMID_TSL_PCT if pos.pyramid_count > 0 else TRAILING_SL_PCT
        
        # Calculate Combined Premium Status
        total_entry_premium = (pos.ce_entry_price * pos.ce_qty) + (pos.pe_entry_price * pos.pe_qty)
        total_current_premium = (pos.ce_ltp * pos.ce_qty) + (pos.pe_ltp * pos.pe_qty)
        
        # Avoid division by zero if all closed
        if total_entry_premium > 0:
            # If Naked (Pyramided or one leg stopped out), bypass the Gate.
            # Combined logic is only for the HEDGED Strangle state.
            if pos.is_naked:
                gate_open = True
            else:
                combined_loss_pct = (total_current_premium - total_entry_premium) / total_entry_premium
                gate_open = combined_loss_pct >= COMBINED_SL_PCT
                
                if gate_open:
                    logger.warning(f"⚠️ [CORE] Combined Brokerage SL Triggered! Loss: {combined_loss_pct*100:.2f}% >= {COMBINED_SL_PCT*100}%")
        else:
            gate_open = False
        ...
        if not pos.ce_sl_hit and pos.ce_qty > 0:
            ...
            # C. SL Trigger Check
            # Exit if: (Price > SL AND Gate Open) OR (Price > Hard Max SL)
            individual_sl_breach = pos.ce_ltp >= pos.ce_sl_price
            hard_sl_breach = pos.ce_ltp >= (pos.ce_entry_price * (1 + MAX_LEG_SL_PCT))
            
            should_exit = (individual_sl_breach and gate_open) or hard_sl_breach
            
            if should_exit:
                exit_reason = "Hard SL" if hard_sl_breach else "Combined Gate SL"
                ...
                if pos.ce_qty > pos.initial_ce_qty and pos.initial_ce_qty > 0:
                     ...
                     logger.info(f"🔽 [CORE] CE Pyramid Reduction triggered ({exit_reason})! Exiting extra {reduction_qty} lots.")
                    ...
                    logger.info(f"🛑 [CORE] CE Stop Loss Hit ({exit_reason})! {pos.ce_ltp} >= {pos.ce_sl_price:.2f}")
        ...
        if not pos.pe_sl_hit and pos.pe_qty > 0:
            ...
            # C. SL Trigger Check
            individual_sl_breach = pos.pe_ltp >= pos.pe_sl_price
            hard_sl_breach = pos.pe_ltp >= (pos.pe_entry_price * (1 + MAX_LEG_SL_PCT))
            ...
            if should_exit:
                ...
                if pos.pe_qty > pos.initial_pe_qty and pos.initial_pe_qty > 0:
                     ...
                     logger.info(f"🔽 [CORE] PE Pyramid Reduction triggered ({exit_reason})! Exiting extra {reduction_qty} lots.")
                    ...
                    logger.info(f"🛑 [CORE] PE Stop Loss Hit ({exit_reason})! {pos.pe_ltp} >= {pos.pe_sl_price:.2f}")
